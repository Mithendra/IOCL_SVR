...
| **1. Daily Sales Entry** — per-pump/shift entry; locked Sell-rate + carried-reading + inventory-opening snapshot; calc engine; **one entry per pump/date/user** (a correction re-opens & edits the same row, no duplicate; re-opens the day's Summary side for re-verification); **Excel import/export** (round-trips, recompute-and-flag); **Scan/Upload** — typed PDFs read from the text layer (reliable), handwriting via Tesseract draft-only (`OCR-findings-2026-09-09.md`). All non-manual paths: never save, ADR-5 review. | done |
