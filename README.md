...
| OCR pipeline (recognition + review), external bank-statement reconciliation | not started — Tesseract **engine** is bundled (ADR-6, `/daily-sales-entry/ocr/status`); the pipeline that uses it is not built (`/ocr` → 501). Excel import/export (module 1), 2FA/TOTP (module 5), and Employee Master insurance (module 8) are **done**. |
