...
| OCR pipeline (recognition + review), external bank-statement reconciliation, Employee Master insurance sections | not started — Tesseract **engine** is bundled (ADR-6, `/daily-sales-entry/ocr/status`); the pipeline that uses it is not built (`/ocr` → 501). Excel import/export (module 1) and 2FA/TOTP (module 5) are **done**. |
