# HANDOFF — install build 0.1.6 on the remote PC and run the two-day test
...
```
C:\Mithendra\SVR\installer\output\SVR-IOCL-Station-Setup-0.1.6.exe
```
...
- The previously installed build is **0.1.1**. Install 0.1.6 straight over it —
...
2. Right-click `SVR-IOCL-Station-Setup-0.1.6.exe` → **Run as administrator**.
...
`D52`). SEP16's 4.1 then carries itself across from SEP15's close — 0.1.6 is
...
Add `### 5.13 Results (remote PC, 2026-09-16, build 0.1.6)` to `HANDOVER.md`:
