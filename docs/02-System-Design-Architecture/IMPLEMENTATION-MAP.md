...
| Print | Print / Print Blank cover Sections 1-7 only, matching the physical paper form - Section 8 "Daily Summary" (an internal report, not on the paper form) and the operational banners/status line are hidden under `@media print` (`#daily-summary-block`, `#carried-note`, `#editing-note`, `#save-status`, `#toolbar-hint`). `@page` is **A4 portrait** (`app.css`, fixed 2026-09-11) - the client's own reference blank forms (`SVR_DSR_EMPTY_<serial>.pdf`) are single-page A4 portrait; the prior forced-landscape setting fought that badly enough that one pump's form didn't print at all. A table row / section heading is kept from splitting across a page break (`break-inside: avoid`) so a half-printed row can't be mistaken for nothing printing. This rule is in the shared `@media print` block, so every module's print output is now portrait, not just Daily Sales Entry. |
| Net Bal Hand Off formula | **Client-corrected 2026-09-11**: the original mockup formula omitted Phone Pay Settled - confirmed a real gap, not intentional. Now `(gas_total + oil_total) − expenses_total + phone_pay_settled + phone_pay_not_settled + new_credits_total + credit_cards_total + night_cash`, in both `calc/daily_sales_entry.py` (authoritative) and `lib/calc-mirror.js` (UX mirror) - kept in step per SDD §6.4/§7.3. The on-screen Summary label and `skills/daily-sales-entry/references/formula-register.md` updated to match. |
